...
public class ib_fenceVar : MonoBehaviour
{
    ...
    {                               
        ...
        localHoehe = dick;
        Vector3 e = new Vector3(dick, localHoehe, length) + pos;  // Decke: entgegen dem Uhrzeigersinn 
        Vector3 f = new Vector3(-dick, localHoehe, length) + pos;
        Vector3 g = new Vector3(-dick, localHoehe, 0) + pos;
        Vector3 h = new Vector3(dick, localHoehe, 0) + pos;
        createStakeFaces(a, d, c, b);                    // Zusammensetzen des Wuerfels / Pfahls
        createStakeFaces(e, f, g, h);
        createStakeFaces(a, e, h, d);
        createStakeFaces(b, c, g, f);
  ...
  void OnCollisionEnter(Collision otherObj)
    {
        if (otherObj.gameObject.tag == "Player" || otherObj.gameObject.tag == "PlayerColl")
        {
            if (touch != true)
            {
                // minuspunkte += abzug;
                ib_StaticVar._score += abzug;
                Debug.Log("Punktabzug: 'Autsch, das war der Zaun!'");
                touch = true;

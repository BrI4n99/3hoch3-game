...
public class ib_startScript : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        Vector3[] positions = new Vector3[3] { new Vector3(rand, 0, 100), new Vector3(rand, 0, 338-180), new Vector3(rand, 0, 180) };

        ib_balesMethPos meth;
 
        meth = ib_balesMethPos.Instance;
        int i = 2; // Anzahl der Hindernisse
        meth.einfach(ib_balesMethPos.posX[Random.Range(0, 3)] , ib_balesMethPos.posY[0], ib_balesMethPos.abstand * i, true, false, gameObject);  i++;
       ...
       for (int j = 0; j < positions.Length; j++) {
        ...
        newcarrot2.transform.Translate(new Vector3(-24, 0, 25));
